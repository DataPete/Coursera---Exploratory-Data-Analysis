...
#------------------------------------------------------------------------------#
# Function reads the data, creates a histogram using the Global_active_power
# column.  Width and height are set to 480, and the image is copied as a png
# file.  PNG file is saved in current working directory.
#------------------------------------------------------------------------------#
read_Write_Hist <- function() {
    # Read household power consumption text file
    data <- read.csv.sql("household_power_consumption.txt", 
                         sql = "select * from file where Date in ('1/2/2007',
                         '2/2/2007')", header = TRUE, sep = ";")
    
    # Sets labels for title, x-axis, and sets a variable for the desired column
    title <- "Global Active Power"
    x.Label <- "Global Active Power (kilowatts)"
    col.Gap <- data$Global_active_power
    
    # Creates png file titled plot1
    png("plot1.png", width = 480, height = 480)
    
    # Creates histogram
    hist(col.Gap, col = "red", main = title, xlab = x.Label)
